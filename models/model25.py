...
from models.skip_transformer2 import SkipTransformer
...
class PointNet_SD(nn.Module):
    def __init__(self, class_num):
        ...
        self.sa_module_11 = PointNet_SA_Module_KNN(256, 16, 3, [64, 128], group_all=False, if_bn=False, if_idx=True)
        self.sa_module_21 = PointNet_SA_Module_KNN(256, 16, 3, [64, 128], group_all=False, if_bn=False, if_idx=True)
        self.sa_module_31 = PointNet_SA_Module_KNN(512, 32, 3, [64, 128], group_all=False, if_bn=False, if_idx=True)
        self.sa_module_12 = PointNet_SA_Module_KNN(128, 8, 128, [128, 256], group_all=False, if_bn=False, if_idx=True)
        self.sa_module_22 = PointNet_SA_Module_KNN(128, 8, 128, [128, 256], group_all=False, if_bn=False, if_idx=True)
        self.sa_module_32 = PointNet_SA_Module_KNN(256, 16, 128, [128, 256], group_all=False, if_bn=False, if_idx=True)
        self.sa_module_13 = PointNet_SA_Module_KNN(None, None, 256, [256, 512], group_all=True, if_bn=False)
        self.sa_module_23 = PointNet_SA_Module_KNN(None, None, 256, [256, 512], group_all=True, if_bn=False)
        self.sa_module_33 = PointNet_SA_Module_KNN(None, None, 256, [256, 512], group_all=True, if_bn=False)
    ...
    def forward(self, pcds):
        l0_xyz = pcds[0].permute(0, 2, 1).contiguous()
        l1_xyz = pcds[1].permute(0, 2, 1).contiguous()
        l2_xyz = pcds[2].permute(0, 2, 1).contiguous()
        l3_xyz = pcds[3].permute(0, 2, 1).contiguous()   
        l0_points = pcds[0].permute(0, 2, 1).contiguous()
        l1_points = pcds[1].permute(0, 2, 1).contiguous()
        l2_points = pcds[2].permute(0, 2, 1).contiguous()
        l3_points = pcds[3].permute(0, 2, 1).contiguous()   
        # print(f'PointNet input {l0_xyz.shape, l1_xyz.shape, l2_xyz.shape, l3_xyz.shape}')

        # l1_xyz, l1_points, idx1 = self.sa_module_1(l1_xyz, l0_points)  # (B, 3, 512), (B, 128, 512)
        # print(f'PointNet 1 {l1_points.shape}')
        l11_xyz, l11_points, idx11 = self.sa_module_11(l1_xyz, l1_points)  # (B, 3, 256), (B, 128, 256)
        l12_xyz, l12_points, idx12 = self.sa_module_21(l2_xyz, l2_points)  # (B, 3, 256), (B, 128, 256)
        # l12_xyz, l12_points = torch.cat((l11_xyz, l12_xyz), 2), torch.cat((l11_points, l12_points), 2) # (B, 3, 512), (B, 128, 512)
        l13_xyz, l13_points, idx13 = self.sa_module_31(l3_xyz, l3_points)  # (B, 3, 512), (B, 128, 512)
        # print(f'PointNet 2 {l21_points.shape}')
        l21_xyz, l21_points, idx21 = self.sa_module_12(l11_xyz, l11_points)  # (B, 3, 128), (B, 256, 128)
        l22_xyz, l22_points, idx22 = self.sa_module_22(l12_xyz, l12_points)  # (B, 3, 128), (B, 256, 128)
        # l22_xyz, l22_points = torch.cat((l21_xyz, l22_xyz), 2), torch.cat((l21_points, l22_points), 2) # (B, 3, 256), (B, 256, 256)
        l23_xyz, l23_points, idx23 = self.sa_module_32(l13_xyz, l13_points)  # (B, 3, 256), (B, 256, 256)
        # l23_xyz, l23_points = torch.cat((l22_xyz, l23_xyz), 2), torch.cat((l22_points, l23_points), 2) # (B, 3, 512), (B, 256, 512)

        l31_xyz, l31_points = self.sa_module_13(l21_xyz, l21_points)  # (B, 3, 1), (B, out_dim, 1)
        l32_xyz, l32_points = self.sa_module_23(l22_xyz, l22_points)  # (B, 3, 1), (B, out_dim, 1)
        l33_xyz, l33_points = self.sa_module_33(l23_xyz, l23_points)  # (B, 3, 1), (B, out_dim, 1)
        # print(f'PointNet 3 {l31_points.shape, l32_points.shape, l33_points.shape}')
...
class PointNet_SD_DOH(nn.Module):
    def __init__(self, class_num):
        print('Downstream = PointNet-SD')
        super(PointNet_SD_DOH, self).__init__()
        ...
        self.cls3 = nn.Linear(128, class_num)
...
class PointNet_SD_Cascade(nn.Module): # will collapse
    def __init__(self, class_num):
        print('Downstream = PointNet-SD-Cascade')
        super(PointNet_SD_Cascade, self).__init__()
        self.repres = {0: [], 1: [], 2: [], 3: []} # 0: in, 1~3: inter rep, 4: out  , 4: []
        self.logits = {0: [], 1: [], 2: [], 3: []} # 0: in, 1~3: inter rep, 4: out  , 4: []
        self.xyzs = {0: [], 1: [], 2: [], 3: []}
        self.sa_module_1 = PointNet_SA_Module_KNN(512, 12, 3, [64, 128], group_all=False, if_bn=False, if_idx=True)
        self.sa_module_12 = PointNet_SA_Module_KNN(128, 8, 128, [128, 256], group_all=False, if_bn=False, if_idx=True)
        self.sa_module_13 = PointNet_SA_Module_KNN(None, None, 256, [256, 512], group_all=True, if_bn=False)
    ...
    def gather_info(self, ):
        for i in range(4):
            self.repres[i] = np.concatenate(self.repres[i])#[:600]
            self.logits[i] = np.concatenate(self.logits[i])#[:600]
            self.xyzs[i] = np.concatenate(self.xyzs[i])#[:600]
            # print(f'gather {self.repres[i].shape}')

    def reset(self):
        self.repres = {0: [], 1: [], 2: [], 3: []} #, 4: []
        self.logits = {0: [], 1: [], 2: [], 3: []} #, 4: []
        self.xyzs = {0: [], 1: [], 2: [], 3: []}
    
    def forward(self, point_cloud, cls_label=None):
        l0_xyz = point_cloud[-1].permute(0, 2, 1).contiguous()
        l0_points = point_cloud[-1].permute(0, 2, 1).contiguous()
        self.repres[0].append(point_cloud[-1].detach().cpu().numpy()) # .reshape(len(l0_points), -1)
        self.logits[0].append(point_cloud[-1].detach().cpu().numpy()) # .reshape(len(l0_points), -1)
        self.xyzs[0].append(point_cloud[-1].detach().cpu().numpy())
        # print(f'PointNet point_cloud {point_cloud.shape}')
        """1"""
        # l1_xyz, l1_points, _ = self.sa_module_1(l0_xyz, l0_points)  # (B, 3, 512), (B, 128, 512)
        # l21_xyz, l21_points, _ = self.sa_module_21(l1_xyz, l1_points)  # (B, 3, 256), (B, 128, 256)
        # l22_xyz, l22_points, _ = self.sa_module_22(l1_xyz, l1_points)  # (B, 3, 256), (B, 128, 256)
        # l23_xyz, l23_points, _ = self.sa_module_23(l1_xyz, l1_points)  # (B, 3, 256), (B, 128, 256)
        # l31_xyz, l31_points = self.sa_module_31(l21_xyz, l21_points)  # (B, 3, 128), (B, 256, 128)
        # l32_xyz, l32_points = self.sa_module_32(l22_xyz, l22_points)  # (B, 3, 64), (B, 542, 64)
        # l33_xyz, l33_points = self.sa_module_33(l23_xyz, l23_points)  # (B, 3, 1), (B, out_dim, 1)
        """2"""
        l1_xyz, l1_points, _ = self.sa_module_1(l0_xyz, l0_points)  # (B, 3, 512), (B, 128, 512)
        l12_xyz, l12_points, _ = self.sa_module_12(l1_xyz, l1_points)  # (B, 3, 256), (B, 128, 256)
        l13_xyz, l13_points = self.sa_module_13(l12_xyz, l12_points)  # (B, 3, 1), (B, 128, 1)
        l22_xyz, l22_points, _ = self.sa_module_22(l1_xyz, l1_points)  # (B, 3, 256), (B, 256, 256)
        l23_xyz, l23_points = self.sa_module_23(l22_xyz, l22_points)  # (B, 3, 1), (B, out_dim, 1)
        l32_xyz, l32_points, _ = self.sa_module_32(l22_xyz, l22_points)  # (B, 3, 256), (B, 256, 256)
        l33_xyz, l33_points = self.sa_module_33(l32_xyz, l32_points)  # (B, 3, 1), (B, out_dim, 1)
        # self.repres[1].append(l12_points.cpu().detach().numpy().reshape(len(l0_points), -1))
        # self.repres[2].append(l22_points.cpu().detach().numpy().reshape(len(l0_points), -1))
        # self.repres[3].append(l32_points.cpu().detach().numpy().reshape(len(l0_points), -1))
        """3"""
        # print(f'PointNet 6 {l33_xyz.shape, l33_points.shape}')
        self.xyzs[1].append(l12_xyz.detach().cpu().numpy())
        self.xyzs[2].append(l22_xyz.detach().cpu().numpy())
        self.xyzs[3].append(l32_xyz.detach().cpu().numpy())
        
        self.repres[1].append(l12_points.detach().cpu().numpy())
        self.repres[2].append(l22_points.detach().cpu().numpy())
        self.repres[3].append(l32_points.detach().cpu().numpy())

        l13_points = l13_points.squeeze(2)
        l23_points = l23_points.squeeze(2)
        l33_points = l33_points.squeeze(2)
        self.logits[1].append(l13_points.detach().cpu().numpy())
        self.logits[2].append(l23_points.detach().cpu().numpy())
        self.logits[3].append(l33_points.detach().cpu().numpy())
        l13_points = self.fc1(l13_points)
        l23_points = self.fc2(l23_points)
        l33_points = self.fc3(l33_points)
        logit1 = self.cls1(l13_points)
        logit2 = self.cls2(l23_points)
        logit3 = self.cls3(l33_points)
        # self.repres[1].append(logit1.cpu().detach().numpy())
        # self.repres[2].append(logit2.cpu().detach().numpy())
        # self.repres[3].append(logit3.cpu().detach().numpy())
        return [logit1, logit2, logit3], [l13_points, l23_points, l33_points]


class PointNet_SD_Seg(nn.Module): # will collapse
    def __init__(self, class_num):
        ...
        self.fc1 = ConvLayer(128)
        self.fc2 = ConvLayer(128)
        self.fc3 = ConvLayer(128)
        self.cls1 = nn.Conv1d(128, class_num, 1)
        self.cls2 = nn.Conv1d(128, class_num, 1)
        self.cls3 = nn.Conv1d(128, class_num, 1)
    ...
    def forward(self, point_cloud, cls_label):
        ...
        l10_points = self.fc1(l10_points)
        l20_points = self.fc2(l20_points)
        l30_points = self.fc3(l30_points)
        logit1 = self.cls1(l10_points)
        logit2 = self.cls2(l20_points)
        logit3 = self.cls3(l30_points)
        return [logit1, logit2, logit3], [l10_points, l20_points, l30_points]
...
class PointNet_SD_Ensemble(nn.Module):
    def __init__(self, class_num):
        ...
        self.sa_module_1 = PointNet_SA_Module_KNN(512, 16, 3, [64, 128], group_all=False, if_bn=False, if_idx=True)
        self.sa_module_21 = PointNet_SA_Module_KNN(512, 8, 128, [128, 256], group_all=False, if_bn=False, if_idx=True)
        self.sa_module_22 = PointNet_SA_Module_KNN(512, 16, 128, [128, 256], group_all=False, if_bn=False, if_idx=True)
        self.sa_module_23 = PointNet_SA_Module_KNN(512, 32, 128, [128, 256], group_all=False, if_bn=False, if_idx=True)
        self.sa_module_31 = PointNet_SA_Module_KNN(None, None, 256, [512, 512], group_all=True, if_bn=False)
...
class PointMLP_SD(nn.Module):
    def __init__(self, points=1024, class_num=40, embed_dim=64, groups=1, res_expansion=1.0,
        ...
        # for i in range(len(pre_blocks)):
        #     self.embeddings.append(nn.Sequential(
        #     nn.Conv1d(in_channels=3, out_channels=embed_dim, kernel_size=1, bias=True),
        #     nn.BatchNorm1d(embed_dim),
        #     nn.ReLU(inplace=True),
        # ))
        ...
        for i in range(len(pre_blocks)):
            out_channel = last_channel * dim_expansion[i]
            self.embeddings.append(nn.Sequential(
            nn.Conv1d(in_channels=3, out_channels=last_channel, kernel_size=1, bias=True),
    ...
    def forward(self, pcds):
        ...
        xyzs, xs = pcds[1:], []
        # batch_size, _, _ = x.size()
        # x = self.embedding(pcds[1])  # B,D,N
        inter_x = []
        for i in range(self.stages):
            # added by zky, parallely, xyz[b, 3, p]
            x = self.embeddings[i](xyzs[i].permute(0, 2, 1))
            # print(f'i {i}, x {x.shape}, xyz {xyzs[i].shape}')
            # Give xyz[b, p, 3] and fea[b, p, d], return new_xyz[b, g, 3] and new_fea[b, g, k, d]
            xyz, x = self.local_grouper_list[i](xyzs[i], x.permute(0, 2, 1))  # [b,g,3]  [b,g,k,d]
...
class PointMLP_SD_Cascade(nn.Module):
    def __init__(self, points=1024, class_num=40, embed_dim=64, groups=1, res_expansion=1.0,
                 activation="relu", bias=True, use_xyz=True, normalize="center",
                 dim_expansion=[2, 2, 2, 2], pre_blocks=[2, 2, 2, 2], pos_blocks=[2, 2, 2, 2],
                 k_neighbors=[32, 32, 32, 32], reducers=[2, 2, 2, 2], **kwargs):
        print('Downstream = PointMLP-SD_Cascade')
        super(PointMLP_SD_Cascade, self).__init__()
        self.stages = len(pre_blocks)
        self.class_num = class_num
        self.points = points
        self.embedding = nn.Sequential(
        ...
        for i in range(len(pre_blocks)):
            ...
            last_channel = out_channel
            fc_in_channels.append(last_channel)
            # print(f'last_channel2: {last_channel}') # 128, 256, 512
        ...
        self.fc1 = FCLayer(fc_in_channels[0])
        self.fc2 = FCLayer(fc_in_channels[1])
        self.fc3 = FCLayer(fc_in_channels[2])
        self.cls1 = nn.Linear(128, class_num)
        self.cls2 = nn.Linear(128, class_num)
        self.cls3 = nn.Linear(256, class_num)

    def forward(self, pcd, cls_label=None):
        xyz = pcd[-1]
...
class CurveNet_SD(nn.Module):
    def __init__(self, num_classes=40, k=20, setting='default'):
        ...
        # self.cic31 = CIC(npoint=256, radius=0.05, k=k, in_channels=64, output_channels=128, bottleneck_ratio=2, mlp_num=1, curve_config=curve_config[setting][2])
        # self.cic32 = CIC(npoint=256, radius=0.1, k=k, in_channels=128, output_channels=128, bottleneck_ratio=4, mlp_num=1, curve_config=curve_config[setting][2])

        # self.cic41 = CIC(npoint=64, radius=0.1, k=k, in_channels=128, output_channels=256, bottleneck_ratio=2, mlp_num=1, curve_config=curve_config[setting][3])
        # self.cic42 = CIC(npoint=64, radius=0.2, k=k, in_channels=256, output_channels=256, bottleneck_ratio=4, mlp_num=1, curve_config=curve_config[setting][3])
    ...
    def forward(self, xyz, return_feats=False):
        ...
        l2_xyz, l2_points = self.cic21(l1_xyz, l1_points)
        l2_xyz, l2_points = self.cic22(l2_xyz, l2_points)
        # print(f'l2_xyz, l2_points: {l2_xyz.shape, l2_points.shape}')
        # print(f'1 x_max, x_avg: {x_max1.shape, x_avg1.shape}')
        ...
        l3_xyz, l3_points = self.cic31(l2_xyz, l2_points)
        l3_xyz, l3_points = self.cic32(l3_xyz, l3_points)
        # print(f'l3_xyz, l3_points: {l3_xyz.shape, l3_points.shape}')
        x2 = self.conv02(l3_points)
        x_max2 = F.adaptive_max_pool1d(x2, 1)
        x_avg2 = F.adaptive_avg_pool1d(x2, 1)
        # print(f'2 x_max, x_avg: {x_max2.shape, x_avg2.shape}')

        x2 = torch.cat((x_max2, x_avg2), dim=1).squeeze(-1)

        l4_xyz, l4_points = self.cic41(l3_xyz, l3_points)
        l4_xyz, l4_points = self.cic42(l4_xyz, l4_points)
        # print(f'l4_xyz, l4_points: {l4_xyz.shape, l4_points.shape}')

        x3 = self.conv03(l4_points)
        x_max3 = F.adaptive_max_pool1d(x3, 1)
        x_avg3 = F.adaptive_avg_pool1d(x3, 1)
        # print(f'3 x_max, x_avg: {x_max3.shape, x_avg3.shape}')

        x3 = torch.cat((x_max3, x_avg3), dim=1).squeeze(-1)
        if return_feats:
            return x3, None
        # print(f'3 x1, x2, x4: {x1.shape, x2.shape, x3.shape}')

        logit1 = self.fc1(x1)
        logit2 = self.fc2(x2)
        logit3 = self.fc3(x3)

        return [logit1, logit2, logit3], [l1_points, l3_points, l4_points]


class CurveNet_SD_Ensemble(nn.Module):
    def __init__(self, num_classes=40, k=20, setting='default'):
        print('Downstream = CurveNet-SD-Ensemble')
        super(CurveNet_SD_Ensemble, self).__init__()
    ...
    def forward(self, xyz, return_feats=False):
        ...
        return [logit1, logit2, logit3, logit4], [l2_points, l3_points, l4_points]
...
class SPD(nn.Module):
    def __init__(self, dim_feat=512, up_factor=2, i=0, radius=1, dim_cat=2):
        ...
        self.mlp_noise = MLP_CONV(in_channel=3, layer_dims=[64, 3])
    ...
    def forward(self, pcd_prev, feat_global, K_prev=None):
        ...
        # b, _, n_cur = pcd_child.shape
        # noise = torch.normal(mean=0, std=torch.ones((b, 3, n_cur), dtype=torch.float, device=pcd_child.device) * 0.01)
        # noise = torch.tanh(self.mlp_noise(noise))# * 1.0 / 10 #** 3#(self.step - 1)

        pcd_child = pcd_child + delta# + noise
...
class Decoder(nn.Module):
    def __init__(self, dim_feat=512, num_pc=256, num_p0=512, radius=1, up_factors=None, dim_cat=8):
        ...
        # self.deep_cls = PointNet_SD(dim_cat) # pointnet2 SD, parallel, use all pcds
        self.deep_cls = PointNet_SD_Cascade(dim_cat) # pointnet2 SD Cascade, use pcd[-1] 
        # self.deep_cls = PointNet_SD_Seg(dim_cat) # pointnet2 SD Cascade, use pcd[-1] 
        # self.deep_cls = PointNet_SD_DOH(dim_cat) # pointnet2 SD, parallel, use all pcds
        # self.deep_cls = PointNet_SD_Ensemble(dim_cat) # pointnet2 SD Ensemble, 4 outputs
        # self.deep_cls = CurveNet_SD(dim_cat, k=8) # curvenet SD
        # self.deep_cls = CurveNet_SD_Ensemble(dim_cat) # curvenet SD Ensemble, 4 outputs
        # self.deep_cls = PointMLP_SD_Cascade(points=512, class_num=dim_cat, embed_dim=64, groups=1, res_expansion=1.0,
        #            activation="relu", bias=False, use_xyz=False, normalize="anchor",
        #            dim_expansion=[2, 2, 2], pre_blocks=[2, 2, 2], pos_blocks=[2, 2, 2],
        #            k_neighbors=[8, 16, 24], reducers=[2, 2, 2])   # pointmlp, default SD
    ...
    def forward(self, feat, partial, cls_label=None, return_P0=False):
        ...
        """
        # print(f'Decoder input: {feat.shape}')
        b, npoint, _ = partial.shape
        device = partial.device
        arr_pcd, pred_labels, cls_feats = [], [], []
        pcd = self.decoder_coarse(feat).permute(0, 2, 1).contiguous()  # seed: (B, num_pc, 3), 256
        # arr_pcd.append(pcd)
        pcd = fps_subsample(torch.cat([pcd, partial], 1), self.num_p0) # seed_fps: (B, num_p0, 3), 512
        ...
        x_prime = pcd.permute(0, 2, 1).contiguous().detach().cpu().numpy()
        # print(f'x_prime: {x_prime.shape}')
        self.pcd_out.append(x_prime) #.reshape(len(partial), -1)
...
class SnowflakeNet(nn.Module):
    ...
    def forward(self, point_cloud, cls_label=None, return_P0=False):
        ...
        out, pred_labels, cls_feats = self.decoder(code, pcd_bnc, cls_label, return_P0=True)
        # print(f'SnowflakeNet code: {code.shape, cls_feats[-1].shape}')
        return out, pred_labels, [code]+cls_feats
