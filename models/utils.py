...
class MLP_CONV(nn.Module): # > 1 layer
    def __init__(self, in_channel, layer_dims, bn=None):
...
class PointNet_FP_Module(nn.Module):
    ...
    def forward(self, xyz1, xyz2, points1, points2):
        ...
        # print(f'FP points1: {points1.shape}')
        # print(f'FP interpolated_points: {interpolated_points.shape}')
...
def fps_subsample_idx(pcd, n_points=2048):
    """
    Args
        pcd: (b, 16384, 3)
    ...
    returns
        new_pcd: (b, n_points, 3)
    """
    fps_idx = furthest_point_sample(pcd, n_points)
    new_pcd = gather_operation(pcd.permute(0, 2, 1).contiguous(), fps_idx)
    new_pcd = new_pcd.permute(0, 2, 1).contiguous()
    return new_pcd, fps_idx
