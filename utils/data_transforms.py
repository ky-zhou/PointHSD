...
class Compose(object):
    ...
    def __call__(self, data): # data: dict type
        # print(3)
                ...
                for k, v in data.items(): #
                    if k in objects and k in data:
                        if transform.__class__ in [
                            ...
                            # print(f'class= {transform.__class__}, k={k}, v={v.shape}')
                            data[k] = transform(v)
...
class RescaleSegLabels(object):
    ...
    def __call__(self, label):
        ...
        need = self.n_points - curr
        np.random.seed(curr)
        # print(f'need: {need}, curr; {curr}, label: {label.shape}')

        if need < 0:
            return label[np.random.permutation(self.n_points)]

        while curr <= need:
            # print(f'curr <= need. need: {need}, curr; {curr}, label: {label.shape}')
            label = np.tile(label, (2))
            # print(f'curr <= need2. {label.shape}')
            need -= curr
            curr *= 2

        choice = np.random.permutation(need)
        # print(f'choice: {len(choice)}, label: {label.shape, label[choice].shape}')
        label = np.concatenate((label, label[choice]))

        return label


class RescalePointsAndLabels(object):
    def __init__(self, parameters):
        self.n_points = parameters['n_points']

    def __call__(self, ptcloud, label):
        curr = label.shape[0]
        need = self.n_points - curr
        np.random.seed(need)

        if need < 0:
            return ptcloud[np.random.permutation(self.n_points)], label[np.random.permutation(self.n_points)]
