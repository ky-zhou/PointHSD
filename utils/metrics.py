...
class Metrics(object):
    ...
    @classmethod
    def _get_acc(cls, pred, gt):
        pred = pred.argmax(-1)
        acc = (pred == gt).sum() / float(gt.size(0))
        print(f'===accABCDEFG {acc}')
        return acc
...
def compute_overall_iou(pred, target, num_classes):
    shape_ious = []
    pred = pred.max(dim=1)[1]    # (batch_size, num_points)  the pred_class_idx of each point in each sample
