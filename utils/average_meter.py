...
class AverageMeter(object):
    ...
    def __init__(self, items=None):
        # print(f'AverageMeter items: {items}')
        self.items = items
    ...
    def update_batch(self, values, batch_size): # for accuracy type of metrics, ensuring correct when last batch size is different from previous ones
        if type(values).__name__ == 'list':
            for idx, v in enumerate(values):
                self._val[idx] = v
                self._sum[idx] += v
                self._count[idx] += batch_size
        else:
            self._val[0] = values
            self._sum[0] += values
            self._count[0] += batch_size
