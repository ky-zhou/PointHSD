...
def plot_all_epochs_calc_only(gen_data, I_XT_array, I_TY_array, axes, epochsInds, f, index_i, index_j, size_ind,
                    font_size, y_ticks, x_ticks, colorbar_axis, title_str, axis_font, bar_font, save_name, plot_error = True,index_to_emphasis=1000):
    """Plot the infomration plane with the epochs in diffrnet colors """
    #If we want to plot the train and test error
    if plot_error:
    ...
    print(f'epochsInds {epochsInds}')
    # plt.axhline(y=0, color='r', linestyle='-')
    # plt.axvline(x=0, color='r', linestyle='-')

    cmap = plt.get_cmap('gnuplot')
    #For each epoch we have diffrenet color
    print(4, epochsInds[max_index-1]+1)
    colors = [cmap(i) for i in np.linspace(0, 1, epochsInds[max_index-1]+1)]
    #Change this if we have more then one network arch
    nums_arc= -1
    #Go over all the epochs and plot then with the right color
    for index_in_range in range(0, max_index):
        ...
        TY = I_TY_array[index_in_range, :]
        #If this is the index that we want to emphsis
        if (epochsInds[index_in_range]+1) % index_to_emphasis == 0:
            axes[index_i, index_j].plot(XT, TY, marker='o', linestyle=None, markersize=12, markeredgewidth=0.08,
                                        linewidth=2.1,
                                        color=colors[int(epochsInds[index_in_range])],zorder=10)
        else:
            # print('No emphasize')
            axes[index_i, index_j].plot(XT[:], TY[:], marker='o', linestyle='-', markersize=6, markeredgewidth=0.04, linewidth=0.2,
                            color=colors[int(epochsInds[index_in_range])])
    utils.adjustAxes(axes[index_i, index_j], axis_font=axis_font, title_str=title_str, x_ticks=x_ticks,
                     y_ticks=y_ticks, x_lim=[0, 15], y_lim=[0, 5],
                     set_xlabel=index_i == axes.shape[0] - 1, set_ylabel=index_j == 0, x_label='$I(X;Z)$',
                     y_label='$I(Y;Z)$', set_xlim=False,
                     set_ylim=False, set_ticks=False, label_size=font_size)
    #Save the figure and add color bar
    if index_i ==axes.shape[0]-1 and index_j ==axes.shape[1]-1:
        utils.create_color_bar(f, cmap, colorbar_axis, bar_font, epochsInds, title='Epochs')
        f.savefig(save_name+'.jpg', dpi=500, format='jpg', bbox_inches='tight')
        f.savefig(save_name+'.pdf', format="pdf", bbox_inches='tight')
...
def load_figures(mode, str_names=None):
    ...
    if mode == 9:
        ...
        if len(str_names) == 1:
            axes = np.vstack(np.array([axes]))
        f.subplots_adjust(left=0.097, bottom=0.12, right=.87, top=0.99, wspace=0.03, hspace=0.03)
        colorbar_axis = [0.905, 0.08, 0.03, 0.78]
...
def plot_figures(str_names, mode, save_name):
    ...
    # plt.savefig("Info-plane.png")
    plt.show()


def plot_figures_calc_only(str_names, mode, save_name):
    """Plot the data in the given names with the given mode"""
    [font_size, axis_font, bar_font, colorbar_axis, sizes, yticks, xticks,title_strs, f, axes] = load_figures(mode, str_names)
    #Go over all the files
    print('mode', mode)
    for i in range(len(str_names)):
        for j in range(len(str_names[i])):
            ...
            # data  = np.array(data_array['information'])
            I_XT_array = np.array(extract_array(data, 'local_IXT'))
            I_TY_array = np.array(extract_array(data, 'local_ITY'))
            #I_XT_array = np.array(extract_array(data, 'IXT_vartional'))
            #I_TY_array = np.array(extract_array(data, 'ITY_vartional'))
            ...
            if mode ==3:
                ...
                plot_all_epochs_calc_only(data_array, I_XT_array, I_TY_array, axes, epochsInds, f, i, j, sizes[i][j], font_size, yticks, xticks,
                                colorbar_axis, title_strs[i][j], axis_font, bar_font, save_name, plot_error=False, index_to_emphasis=5)


def plot_figures_calc_only_delta(str_names, mode, save_name):
    """Plot the data in the given names with the given mode"""
    [font_size, axis_font, bar_font, colorbar_axis, sizes, yticks, xticks,title_strs, f, axes] = load_figures(mode, str_names)
    #Go over all the files
    print('mode', mode)
    for i in range(len(str_names)):
        for j in range(len(str_names[i])):
            ...
            I_XT_array = np.array(extract_array(data, 'local_IXT'))
            I_TY_array = np.array(extract_array(data, 'local_ITY'))
            I_XT_array_delta = np.vstack((I_XT_array[:, 2] - I_XT_array[:, 0], I_XT_array[:, 2] - I_XT_array[:, 1])).reshape(-1, 2)
            I_TY_array_delta = np.vstack((I_TY_array[:, 2] - I_TY_array[:, 0], I_TY_array[:, 2] - I_TY_array[:, 1])).reshape(-1, 2)
            # I_XT_array_delta = np.diff(I_XT_array, 1)
            # I_TY_array_delta = np.diff(I_TY_array, 1)
            print(I_XT_array.shape, I_XT_array_delta.shape)
            # assert 1==2
            #I_XT_array = np.array(extract_array(data, 'IXT_vartional'))
            #I_TY_array = np.array(extract_array(data, 'ITY_vartional'))
            epochsInds = data_array['params']['epochsInds']
            ...
            if mode ==3:
                ...
                plot_all_epochs_calc_only(data_array, I_XT_array_delta, I_TY_array_delta, axes, epochsInds, f, i, j, sizes[i][j], font_size, yticks, xticks,
                                colorbar_axis, title_strs[i][j], axis_font, bar_font, save_name, plot_error=False, index_to_emphasis=5)
