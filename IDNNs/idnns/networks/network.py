...
def calc_only(load_data, i, j, k, layerSize, num_of_ephocs, indexes,
                               model_type, percent_of_train, calc_information,
                               calc_information_last, num_of_bins,
                               interval_information_display, save_ws, rand_int):
	"""Train the network and calculate it's information"""
	network_name = '{0}_{1}_{2}_{3}'.format(i, j, k, rand_int)
	print ('Calculating info  - {0}'.format(network_name))
	network = load_data()
	network['information'] = []
	if calc_information:
		print ('Calculating the infomration')
		infomration = np.array([inn.get_information_calc_only(network['ws'], network['data'], network['label'],
		                                            num_of_bins, interval_information_display, None,
		                                            layerSize)])
		network['information'] = infomration
	elif calc_information_last:
		print ('Calculating the infomration for the last epoch')
		infomration = np.array([inn.get_information_calc_only([network['ws'][-1]], network['data'], network['label'],
		                                            num_of_bins, interval_information_display,
		                                            None, layerSize)])
		network['information'] = infomration
	# If we dont want to save layer's output
...
def exctract_activity(sess, batch_points_all, model, data_sets_org):
	...
	for i in range(0, len(batch_points_all) - 1):
		...
		# print(len(w_temp_local))
		for s in range(len(w_temp_local[0])): # s max is 6
			# print('is', i, s)
			if i == 0:
